...
// お出かけ(家族)瞬録は、飲み屋向けの二次登録(誰と/また行きたい/予算感/お酒の武器/静かさ)を
// 経由せず、名前確認〜写真1枚〜送信までダイアログ内の1操作で完結する。
test("お出かけを瞬録は写真登録まで一気通貫で完了し、登録待ちに残らない", async ({ page }) => {
  await page.goto("/");
  await page.waitForSelector("#venue-search");

  await page.getByText("お出かけを瞬録").click();
  ...
  // 家族の瞬録は作成と同時にis_completed: trueになるため、登録待ちには出てこない。
  ...
  // 飲み屋向けの項目(お酒の武器・静かさ等)は家族カテゴリでは表示しない。
