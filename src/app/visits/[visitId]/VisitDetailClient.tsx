  ...
  if (!visit.is_completed) {
    return (
      <main className="mx-auto flex max-w-md flex-col gap-4 px-4 pt-8">
        <p className="text-sm text-neutral-500">この記録はまだ盛り付け前です。</p>
        <Link
  ...
  return (
    <main className="mx-auto flex max-w-md flex-col gap-6 px-4 pt-6">
      ...
      <div className="flex gap-3">
        <Link
          href={`/visits/${visit.id}/register`}
          className="flex-1 rounded-full bg-neutral-800 py-3 text-center text-sm font-semibold text-neutral-200"
        >
          編集する
        </Link>
        <button
          ...
          disabled={deleting}
          className="flex-1 rounded-full bg-neutral-800 py-3 text-sm font-semibold text-red-400 disabled:opacity-60"
        >
          {deleting ? "削除中..." : "削除する"}
        </button>
      </div>
